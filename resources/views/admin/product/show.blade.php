    ...
    <x-slot name="header">
        ...
        <div class="mt-3 text-sm">
            <a href="{{ route('dashboard') }}" class="text-blue-500 underline"><i class="bi bi-house"></i></a> -> <a
                href="{{ route('product.index') }}" class="text-blue-500 underline">Products</a> -> <span>Edit
                product</span>
        </div>
    </x-slot>
    ...
    <form action="{{ route('product.update', $product->id) }}" method="post" enctype="multipart/form-data"
        ...
        <div class="mb-4">
            ...
            <textarea type="text" name="description" id="description"
                class="w-full px-4 py-2 border rounded-md focus:outline-none focus:border-blue-500">{{ old('description') ?? $product->description }}</textarea>
        </div>
        <div class="mb-4">
            <label class="block text-gray-700 text-sm font-bold mb-2">Product image:</label>

            <div class="flex items-center" x-data="{ imagePreview: {{ '"' . asset('storage/' . $product->image) . '"' }} }">
        ...
        <div x-data="{ selectedCategories: {{ $product->categories->pluck('id') }} }">
